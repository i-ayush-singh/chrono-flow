...
CURL_COMMON_ARGS=(--connect-timeout 3 --max-time 20 --retry 1 --retry-delay 1)
...
tenant_response="$(curl -sS "${CURL_COMMON_ARGS[@]}" -X POST "$JOB_SERVICE_URL/api/v1/tenants" \
...
key_response="$(curl -sS "${CURL_COMMON_ARGS[@]}" -X POST "$JOB_SERVICE_URL/api/v1/tenants/$tenant_id/api-keys")"
...
job_response="$(curl -sS "${CURL_COMMON_ARGS[@]}" -X POST "$GATEWAY_URL/api/v1/jobs" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $api_credential" \
  -d "$job_payload")"
if ! python3 -c 'import json,sys; d=json.loads(sys.stdin.read()); assert "id" in d' <<<"$job_response" 2>/dev/null; then
  echo "Job create failed or timed out. Raw response:"
  echo "$job_response"
  exit 1
fi
...
list_response="$(curl -sS "${CURL_COMMON_ARGS[@]}" -X GET "$GATEWAY_URL/api/v1/jobs?tenantId=$tenant_id" \
  -H "X-API-Key: $api_credential")"
